...
export default function TocPanel({ onNavigate }: TocPanelProps = {}) {
  ...
  return (
    <div className="flex flex-col h-full bg-surface-dark border-r border-hair">
      {/* Panel Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-hair">
        <span className="text-xs font-semibold tracking-widest text-ink-faint uppercase flex-1">
        ...
        <button
          ...
          className={`min-w-[40px] min-h-[40px] flex items-center justify-center rounded-lg border transition-colors ${
            showSearch
              ...
              : "border-hair bg-white/[0.035] text-ink-faint hover:border-hair hover:bg-white/[0.06] hover:text-ink/65"
          }`}
          title="Search"
        >
          <Search size={14} />
  ...
  if (chapters.length === 0) {
    return (
      <div className="px-4 py-6">
        <p className="text-xs text-ink-faint">No chapters detected.</p>
      </div>
    );
  }
      ...
      {chapters.map((chapter, index) => {
        ...
        return (
          ...
          >
            <ChevronRight
              size={12}
              className={isActive ? "text-lumina-gold flex-shrink-0" : "text-transparent group-hover:text-ink-faint flex-shrink-0 transition-colors"}
            />

            <div className="flex-1 min-w-0">
              <p className={`text-xs leading-snug truncate ${
                isActive ? "text-ink font-medium" : "text-ink-soft group-hover:text-ink-soft"
              }`}>
                {displayTitle.title}
              </p>
              {displayTitle.eyebrow && (
                <p className="mt-0.5 truncate text-[10px] uppercase tracking-[0.16em] text-ink-faint">
                  {displayTitle.eyebrow}
                </p>
              )}
            </div>
  ...
  return (
    <div className="py-2 space-y-px">
      {notes.map((note) => {
        ...
        return (
          <div key={note.id} className="px-4 py-3 hover:bg-ink/5 transition-colors">
            {highlight && (
              <p className="text-xs text-ink-faint line-clamp-2 italic mb-1">
                "{highlight.selectedText}"
              </p>
            )}
            <p className="text-xs text-ink-soft leading-relaxed">{note.noteText}</p>
          </div>
        );
      })}
    </div>
