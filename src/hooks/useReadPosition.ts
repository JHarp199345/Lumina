...
export function useReadPosition(onPositionChange?: (wordPosition: number) => void) {
  ...
  const estimateWordPosition = useCallback((): number => {
    ...
    const { currentChapterIndex, percentComplete } = useReaderStore.getState();

    const wordsBeforeCurrentChapter = activeStructure.chapters
      .slice(0, currentChapterIndex)
      .reduce((sum, ch) => sum + ch.wordCount, 0);

    const currentChapter = activeStructure.chapters[currentChapterIndex];
    const wordsIntoCurrentChapter = currentChapter
      ? currentChapter.wordCount * (percentComplete / 100)
      : 0;

    return Math.round(wordsBeforeCurrentChapter + wordsIntoCurrentChapter);
  }, [activeStructure]);
